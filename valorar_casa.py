...
def pedir_opcion(mensaje, opciones, mostrar_top=20):
    ...
    while True:
        ...
        print(f"   ⚠️  '{entrada}' no encontrado. Opciones:")
...
departamento = pedir_opcion("🔹 Departamento:", departamentos_validos)
...
if usar_coords:
    ...
    print(f"   ℹ️  Usando coordenadas aproximadas de {ciudad}: ({latitud:.2f}, {longitud:.2f})")
...
if area < 60:
    categoria_tamano = 'Pequeña'      # ✓ NOMBRE EXACTO
elif area < 120:
    categoria_tamano = 'Mediana'      # ✓ NOMBRE EXACTO
elif area < 200:
    categoria_tamano = 'Grande'       # ✓ NOMBRE EXACTO
else:
    categoria_tamano = 'Muy Grande'   # ✓ NOMBRE EXACTO (no 'Extra Grande')
...
if 'df' in locals():
    ...
    if precio_estimado_inicial < cuartiles[0]:
        categoria_precio = 'Económica'     # ✓ NOMBRE EXACTO (Q1)
    elif precio_estimado_inicial < cuartiles[1]:
        categoria_precio = 'Media'          # ✓ NOMBRE EXACTO (Q2)
    elif precio_estimado_inicial < cuartiles[2]:
        categoria_precio = 'Alta'           # ✓ NOMBRE EXACTO (Q3)
    else:
        categoria_precio = 'Premium'        # ✓ NOMBRE EXACTO (Q4)
else:
...
# ESTRATEGIA CORRECTA: Combinar con dataset, codificar, extraer última fila
if 'df' in locals() and not df.empty:
    ...
    # Extraer SOLO la última fila (nuestra nueva observación)
    ...
    print("   ⚠️  Advertencia: Dataset no disponible, usando encoding básico")
...
print()
print(f"   ℹ️  Precisión del modelo: MAPE = 0.80% (error promedio de $11M COP)")
print(f"   ℹ️  Confiabilidad: R² = 0.9899 (98.99% de varianza explicada)")
print()
print("="*80)
...
if 'df' in locals():
    ...
    if len(df_similares) > 0:
        ...
        print(f"\n   ⚠️  No hay suficientes propiedades similares en la base de datos")
